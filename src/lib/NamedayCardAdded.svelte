...
<article class="bg-black text-gray-200 rounded p-2 flex justify-between">
  <div>
    <h1>{nameday.name}</h1>
    <p>{nameday.date.day} {monthString}</p>
  </div>
  <button class="p-2" on:click={removeName}><svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 hover:scale-125 transition ease-in-out" viewBox="0 0 20 20" fill="currentColor">
